...
public class CodeClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // DF VIBE fork: always start the local API — the /df tool depends on it, and
        // there is no /auth gate anymore, so there is no reason to leave it off.
        // (The CodeClientAPI config toggle is intentionally ignored here.)
        try {
            API.start();
        } catch (Exception e) {
    ...
    private static void loadFeatures() {
        ...
        feat(new dev.dfonline.codeclient.dev.MissingChestRenderer());
    }
...
//        System.out.println(location.name());
    ...
    public static boolean noClipOn() {
        ...
        // Normally NoClip is only for idle (None) flight. Also allow it for actions that opt in
        // (placement, scanning): they fly the player through the codespace and need the same
        // wall-sanitized server position, or DiamondFire rubber-bands them ("seizure").
        boolean actionAllows = (currentAction instanceof None) || currentAction.wantsNoClip();
        if (!actionAllows) return false;
