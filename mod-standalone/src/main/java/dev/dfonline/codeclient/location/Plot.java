    ...
    public @Nullable BlockPos findFreePlacePos(BlockPos origin) {
        ...
        while (y < 255) {
            while (originX - x <= assumeSize().codeWidth - 2) {
                ...
                // The cell ITSELF must be empty, not just its neighbours. Code lines are spaced 3 apart
                // (a block + 2 air), so every placed column has air on both sides - checking only the
                // neighbours returned already-occupied columns, which made batched placements overlap.
                if (world.getBlockState(pos).isAir()
                        && world.getBlockState(pos.east()).isAir() && world.getBlockState(pos.west()).isAir()) return pos;
            }
            y += 5;
            x = originX - 1;
        }
