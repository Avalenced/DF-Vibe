...
Vibe-code [DiamondFire](https://mcdiamondfire.com) from your editor.

Pull a plot's code into local `.py` files, edit them (you, your AI, whoever), push them back.
All from in-game `/df` commands. Dev mode only, no custom flying, one jar.
...
`/df help` covers the rest. Build it yourself with `cd mod-standalone && ./gradlew build`.
...
DiamondFire / [DFOnline](https://dfonline.dev) community. Full list and licenses in
